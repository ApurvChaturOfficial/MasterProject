...
const AboutUpdate = () => {
	...
	// Redux
	const Redux = {
			state: useSelector((fullState) => fullState.AboutUpdateState),
	...
	const APICalls = {
		...
		AboutUpdateAPICall: () => APIs.AboutUpdateAPI(Redux, id, navigate)
	}
	...
	// Submit Render
	useEffect(() => {
			submitFormObject(Redux, APICalls.AboutUpdateAPICall)
      ...
      <Update 
          data = {{
            header: {
              ...
              back: {
                to: `${FinalRouteName.Content.Sidebar.Model.About.RetrieveRoute}/${id}`
              }
            },
            update: {
              inputs: {

...
const BlogUpdate = () => {
	...
	// Redux
	const Redux = {
			state: useSelector((fullState) => fullState.BlogUpdateState),
	...
	const APICalls = {
		...
		BlogUpdateAPICall: () => APIs.BlogUpdateAPI(Redux, id, navigate)
	}
	...
	// Submit Render
	useEffect(() => {
			submitFormObject(Redux, APICalls.BlogUpdateAPICall)
      ...
      <Update 
          data = {{
            header: {
              ...
              back: {
                to: `${FinalRouteName.Content.Sidebar.Model.Blog.RetrieveRoute}/${id}`
              }
            },
            update: {
              inputs: {

...
const APIs = {
  // List API
  ListAPI: (Redux) => {
    API.Content.Sidebar.Main.Sample4.ListAPI()
    .then(response => {
  ...
  CreateAPI: (Redux, close) => {
    API.Content.Sidebar.Main.Sample4.CreateAPI({
			data: {
        basic_info: {
  ...
  RetrieveAPI: (Redux, id) => {
    API.Content.Sidebar.Main.Sample4.RetrieveAPI({id})
    .then(response => {
      // console.log(response.data);
      const serverResponse = response.data;
  ...
  UpdateAPI: (Redux, id, close) => {
    API.Content.Sidebar.Main.Sample4.UpdateAPI({
      id,
			data: {
        basic_info: {
          title: Redux.state.FormObject.FormValue.title,
  ...
  DeleteAPI: (Redux, id, close) => {
    API.Content.Sidebar.Main.Sample4.DeleteAPI({id})
    .then(response => {
      // console.log(response.data);
      const serverResponse = response.data;
